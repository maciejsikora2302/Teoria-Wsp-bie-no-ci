...
var functionsToRun = undefined;

function decrementor(){
    functionsToRun -= 1;
}

function checker(finisherFunction){
    if(functionsToRun>0){
        setTimeout(function() {
            checker(finisherFunction);
        }, 10);
    }
    else{
        finisherFunction(function(){})
    }
}

function inparallel(parallel_functions, final_function) {
    functionsToRun = parallel_functions.length;
    for(i=0;i<parallel_functions.length;i++){
        parallel_functions[i](decrementor);
    }
    checker(final_function)
}
